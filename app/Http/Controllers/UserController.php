...
class UserController extends Controller
{
    ...
    public function appointment(Request $request)
    {
        ...
        if($current<$max_patient){
            ...
            $appointed_at=$start->addMinutes($slot)->floorMinutes(15);
            session(['app_details'=>[
                'doctor_id'=>$doctor->id,
                'appointed_at'=>$appointed_at->toDateTimeString(),
            ]]);
            return view('book',['doctor'=>$doctor,'appointed_at'=>$appointed_at->format('l jS \\of F Y h:i:s A')]);
        }else{
    ...
    public function checkout(Request $request)
    {
        ...
        return view('success',['appointed_at'=>Carbon::createFromDate($session['appointed_at'])->format('l jS \\of F Y h:i:s A'),'doctor'=>$doctor]);
